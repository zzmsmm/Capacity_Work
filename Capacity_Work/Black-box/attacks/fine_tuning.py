...
from trainer import train, test, whitebox_wm_acc
...
def fine_tune(net, device, criterion, optimizer, scheduler, test_loader_orig, dataset, batch_size, num_epochs, patience,
              savefile, save_model, wm_loader=None, tune_all=True, transfer=True):
    """
    Run Fine-Tuning Attack on model.
    """

    savepath = os.path.join('capacity_log', save_model.split('_')[0] + '_resnet50', save_model.split('_')[2], 'FT')
    ...
    if transfer:
        ...
        transform_train, transform_test = get_data_transforms(target_domain)
        train_set, test_set, valid_set = get_dataset(target_domain, train_db_path, test_db_path, transform_train, transform_test, 0.1, 10)
    ...
    return best_test_acc, avg_valid_losses, best_wm_acc, best_test_acc_orig, best_epoch, history


def fine_tune_whitebox(net, device, criterion, optimizer, scheduler, test_loader_orig, dataset, batch_size, num_epochs, patience,
              savefile, model, b, X, bit_length, tune_all=True, transfer=True):
    """
    Run Fine-Tuning Attack on model.
    """
    if transfer:
    ...
    if transfer:
        ...
        transform_train, transform_test = get_data_transforms(target_domain)
        train_set, test_set, valid_set = get_dataset(target_domain, train_db_path, test_db_path, transform_train, transform_test, 0.1)
        train_loader, test_loader, valid_loader = get_dataloader(train_set, test_set, batch_size, valid_set, shuffle=True)
