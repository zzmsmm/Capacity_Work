...
class ResNet18_cifar10(object):
    ...
    '''checkpoint save'''
    def save(self, acc, epoch, device, save_dir):
        state = {
            'acc': acc,
            'epoch': epoch,
            'model_state_dict': self.net.module.state_dict() if device == 'cuda' else self.net.state_dict(),
        }
        if not os.path.isdir(save_dir):
            os.mkdir(save_dir)
        torch.save(state, os.path.join(save_dir, 'resnet18_cifar10_' + str(epoch) + '.pth'))